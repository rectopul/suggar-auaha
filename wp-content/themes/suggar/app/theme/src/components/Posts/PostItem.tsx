...
export function PostItem({ post }: PostItemProps) {
    const [categories, setCategories] = useState<PostCategoryMain[] | null>(
        null
    );
        ...
        try {
            if (post.categories) {
                const categoryArray = await Promise.all(
                    post.categories.map(async (c) => {
                        try {
                            const searchCategory = await getCategory(c);
                            return searchCategory;
                        } catch (error) {
                            console.log(
                                `Erro ao buscar categoria do post`,
                                error
                            );
                            return null;
                        }
    ...
    return (
        <>
            {categories && (
                <div className="w-full xl:w-6/12 px-0 xl:px-2 my-2">
                        ...
                        <figure className="w-full">
                            {thumbnail && (
                                <img
                                    className="w-full"
                                    src={
                                        thumbnail.media_details.sizes.full
                                            .source_url
                                    }
                                    alt={post.slug}
                                />
                        ...
                        <div className="flex-col justify-start items-start gap-3 inline-flex">
                            <span className="flex items-center">
                                {categories.map((c, k) => (
                                    <a
                                        href={c.link}
                                        className="text-blue-900 mr-1 text-lg font-medium font-['Poppins'] underline"
                                        key={k}
                                    >
                                        {c.name}
                                    </a>
                            ...
                            <h2>
                                <a
                                    className="text-black text-xl font-medium font-pop"
                                    href={post.link}
                                >
                                    {post.title.rendered}
                                </a>
                            ...
                            <span
                                className="text-black text-base font-normal font-pop"
