...
export function MenuCategory() {
    ...
    return (
        <ul className="flex items-center text-xs xl:text-sm font-medium xl:font-semibold font-pop uppercase">
            {infos &&
                infos.menus &&
                Object.entries(infos.menus).map(([, menuItems]) => (
                    <>
                        {menuItems.map((m, k) => (
                            <li
                                className={`px-1.5 xl:px-5 py-1 xl:py-2 ${
