...
import {
    ...
    ProductPros,
    ProductReview,
    ProductWithVariationsPros,
    ShippingTypes,
    ShopTypes,
    UpdateItemCartPayload,
} from "./types";
...
const getShopCredentials = async (): Promise<IntegrationsTypes> => {
    try {
        const req = await fetch(
            ...
            }/wp-json/shop/v1/get-integration-keys`
        );
...
const getCredentialsToLocalStorage = (): IntegrationsTypes => {
    try {
        ...
        const consumer_secret =
            localStorage.getItem("theme_consumer_secret") || "";
...
const getPosts = async (): Promise<PostsMain[]> => {
    try {
        const req = await fetch(
            `${import.meta.env.VITE_LOCAL_SITE_URL}/wp-json/wp/v2/posts`
        );
...
const getShopInfos = async (): Promise<SiteInfoMain> => {
    try {
        const req = await fetch(
            `${import.meta.env.VITE_LOCAL_SITE_URL}/wp-json/shop/v1/infos`
        );
...
const sendNewsLetter = async (
    data: newsPayloadTypes
): Promise<EmailReturnTypes> => {
    try {
        ...
        const req = await fetch(
            `${
                import.meta.env.VITE_LOCAL_SITE_URL
            }/wp-json/contact-form-7/v1/contact-forms/49/feedback`,
            {
                method: "POST",
                headers: {
...
const getNonce = async (): Promise<NonceProps> => {
    try {
        const req = await fetch(
            `${import.meta.env.VITE_LOCAL_SITE_URL}/wp-json/shop/v1/get-nonce`
        );
...
const getCity = async (): Promise<LocationProps> => {
    try {
        const req = await fetch(
            `https://ipinfo.io/json?token=${import.meta.env.VITE_TOKEN_IP}`
        );
...
): Promise<ProductWithVariationsPros[]> => {
    try {
        const req = await fetch(
            `/wp-json/wc/v3/products/${variationId}/variations`
        );
        if (!req.ok) throw Error(`Erro ao requisitar medias`);
...
const retrieveProductVariation = async ({
    product,
    variation,
}: retrieveProdVariationProps): Promise<ProductVariations> => {
    try {
        const req = await fetch(
            `/wp-json/wc/v3/products/${product}/variations/${variation}`
        );
...
const siteConfig = async (): Promise<ShopTypes> => {
    try {
        const req = await fetch(
            `${import.meta.env.VITE_LOCAL_SITE_URL}/wp-json`
        );
...
const productReviews = async (
    ...
    try {
        const req = await fetch(
            `${
                import.meta.env.VITE_LOCAL_SITE_URL
            }/wp-json/wc/store/products/reviews?product_id=${product_id}`
        );
...
const addItemToCart = async ({
    data,
}: AddItemToCartPayload): Promise<CartTypes> => {
    try {
        const url = `${
            import.meta.env.VITE_LOCAL_SITE_URL
...
const updateProductCart = async ({
    data,
}: UpdateItemCartProps): Promise<CartTypes> => {
    try {
        const url = `${
            import.meta.env.VITE_LOCAL_SITE_URL
...
const createOrder = async ({
    data,
}: createOrderProps): Promise<OrderReturnsType> => {
    try {
...
const getPostsByCategory = async (categoryId: number): Promise<PostsMain[]> => {
    try {
        const url = `${
            import.meta.env.VITE_LOCAL_SITE_URL
        ...
        if (!req.ok) throw Error(`Erro ao listar posts da categoria`);
...
const createCommentPost = async ({
    data,
}: CommentProps): Promise<CommentMain> => {
    try {
        const protocol = checkProtocol();
        const url = `${
            !protocol
...
const getAllCatalog = async (
    params?: ParamsGetProductTypes
): Promise<ProductTypes[]> => {
    try {
        const url = params
            ? `${
                  import.meta.env.VITE_LOCAL_SITE_URL
              }/wp-json/wc/v3/products?${objectToQueryString(params)}`
